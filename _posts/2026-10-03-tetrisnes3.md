...
## The Story so Far...
Last Antholog, I switched to using a pre-existing decompilation, rather than try to make my own, and was able to successfully start removing unused game data and adding new piece rotations! Amazingly, by using my old findings and this new decompilation, in two weeks I was able to essentially completely replicate the final state of the school project that took me a couple months. 
...
I thought that moving those pieces around would fix it, but found that only caused other graphical issues. After that, I looked through the code again for anything that resembled sprite data. Then, I found a `pieceToPpuStatAddr` table:

    pieceToPpuStatAddr:
        .dbyt   $2186,$21C6,$2206,$2246
        .dbyt   $2286,$22C6,$2306

I immediately realized that the table used the old piece IDs, and tried just modifying it accordingly. And... that's all it took! Now the score works perfectly!
...
## What's Next?
Now that the statistics are being updated properly, we need to have access to all our blocks. After all, it's hard to keep calling this a Tetris project when we can't even get a Tetris! Adding in the I and O blocks again using the `orientationTable` modifications I talked about last week will be the next logical step.
...
Still, both this project and site has been very rewarding so far! Whether the next post will focus on this project, I'm not certain yet. This site is still pretty new, and I haven't really worked out a proper uploading schedule yet. It's really just been a 'go with the flow' kind of project for now, to help ensure I don't burn out or box myself in too tightly. As for *when* will I start scheduling upload ideas ahead of time? Uhhhhhhhhhhhhhhhhhhh

In other news, I recently finished reading the DragonBall Z manga! My first time ever 'finishing' DragonBall Z was the Abridged series on YouTube, and always wanted to watch the original. But I thought the original had wayyy too much filler, and didn't like all the changes that Kai made. So, I decided to start by just reading the manga! I ATE that thing up. I think I read every issue in less than 3 weeks? Now I'm trying out the original DragonBall manga.
